...
/**
        ...
        @brief considering a point H in segment (Path[i_path-1]..Path[i_path]) corresponding to the
   current position, the def_speed_constraints setted in path and specific speed constraints that
   may be setted in arc nodes (path.dest[node_id].arcs[arc_in_path].speed)
...
static Point projection(Position p, Path* path, int prev_index) {
    ...
    if ((minX <= H.x && H.x <= maxX) && (minY <= H.y && H.y <= maxY)) {
        // YES!
        return H;
    } else {
...
static Bool8 eq_pos(Position p, Node n, Float32 epsilon) {
    Float32 diff_x = p.x - n.x, diff_y = p.y - n.y;
...
static Position get_pose_at_last_node(Node n_1, Node n_2) {
    ...
    p.x = n_2.x;
    p.y = n_2.y;
    p.theta = (n_1.y > n_2.y) ? -acos((n_2.x - n_1.x) / POINTDIST(n_1, n_2))
                              : acos((n_2.x - n_1.x) / POINTDIST(n_1, n_2));
    return p;
}
...
/**
        @brief	GetSetPoint algorithm,
                        ...
                        4. Some points are mandatory and the robot must not deviate at all from
   those points
        ...
        [TODO] (ejn) This algorithm seems a bit complicated. It seems that the constraint about
   distance d could be handled in a simpler way than computing a left and right path. [TODO] (ejn)
   This function has a nasty side effect!!!! (on prev_pos, prev_index; l_side). See if it could be
   avoided! It could be avoided by performing the complete calculation each and every cycle. This
   would be less efficient, but much simpler to analyse in terms of WCET... [TODO] (ejn) Replace the
   returned value (0,1,...) by an enumeration! [TODO] (ejn) Provide a sensible documentation fort
   this function (e.g.: the returned value is not documented).
*/
static int get_setpoint(Position pos_r, Path* path_r, Position* pos_sp) {
    ...
    static Point l_side[MAX_CARTO_NODES], r_side[MAX_CARTO_NODES];

    int ind = prev_index, side_start;
    ...
    float distance = 0.0f, max_range;

    // Compute the point of the path that is the closest to the robot.
    H = projection(pos_r, path_r, prev_index);

    // Compute the maximal distance that could be covered by the robot considering the speed
    // constraints. The *** actual *** distance may be shorter in order to prevent the robot to take
    // "shortcuts" from its current location to the target. (Consider for instance a path with a
    ...
    if ((prev_index == path_r->size - 1) &&
        eq_pos(prev_pos, *(path_r->dest[path_r->size - 1]), max_range)) {
        // YES: the target is the last node.
        *pos_sp = get_pose_at_last_node(*(path_r->dest[path_r->size - 2]),
                                        *(path_r->dest[path_r->size - 1]));
        // We can return.
        return 0;
    }
    ...
    // Move the target setpoint as far as we can, until either the complete range is covered OR the
    // segment (H, target setpoint) intersects the left or right sides (which means that the
    // cross-track constraint is not satisfied. Note that we may also get out of the loop because
    // the
    while ((!flag_range) && (!flag_collision)) {
        ...
        if (!flag_range) { //	NO, we have not covered the maximal range.
            ...
            while (i_side <= ind) {
                ...
                if (i_side > path_r->size - 2) {
                    // Compute the next side point when there is only a unique segment to be
                    // covered.
                    side_lr_points(*(path_r->dest[i_side]), *(path_r->dest[i_side - 1]), _D_,
                                   r_side + i_side, l_side + i_side);
                } else
                    // [TODO] (ejn) This case doesn't seem specific. To be removed?
                    if (i_side == 0) {
                    //
                    side_lr_points(*(path_r->dest[0]), *(path_r->dest[1]), _D_, l_side, r_side);
                } else {
                    // Compute the next side point when there are more than one segment to be
                    // covered.
                    lr_points(*(path_r->dest[i_side - 1]), *(path_r->dest[i_side]),
                              *(path_r->dest[i_side + 1]), _D_, l_side + i_side, r_side + i_side);
                }
                ++i_side;
            }

            // Now that the side paths have been built, check if (H, target) intersects the left or
            // right path.

            if (i_side > 0) {
                // Check if there is an intersection with the "side" path, starting from
                // "side_start" up to i_side.
                flag_collision =
                    test_intersection_point(H, pos, side_start, i_side, l_side, r_side);
            }
            ...
            if (!flag_collision) {
                ...
                // If the node a "fly-by" node and we are not already at that point, the next target
                // is that node. [TODO] (ejn) Beware of those "==" when dealing with floating
                // points... To be checked.
                if ((is_mandatory(npos) && !(H.x == npos.x && H.y == npos.y)) ||
                    ind == (path_r->size - 1)) {
                    ...
                    if (is_init) {
                        ...
                        is_init = false;
                        return 1;
                    } else if ((ind == path_r->size - 1) ||
                               (!(pos_sp->x == prev_pos.x && pos_sp->y == prev_pos.y &&
                                  pos_sp->theta == prev_pos.theta))) {
                        prev_pos = *pos_sp;
                        return 2;
                    }
                }
    ...
    if (flag_collision) {
        ...
        pos_finds_side.theta =
            acos((path_r->dest[ind]->x - H.x) / POINTDIST(H, *(path_r->dest[ind])));
        ...
        // Now that we know the side, compute the farthest point, which is located at the
        // intersection of [H,I] and the next path segment. [TODO] (ejn) This is an approximation:
        // the target could be farther than that.
        if (ind > 0)
            get_intersection(segment(H, side[ind - 1]),
                             segment(node_to_point(*(path_r->dest[ind - 1])),
                                     node_to_point(*(path_r->dest[ind]))),
                             &ip, &status);
        else
            get_intersection(
                segment(H, side[0]),
                segment(node_to_point(*(path_r->dest[0])), node_to_point(*(path_r->dest[ind]))),
                &ip, &status);
        ...
        if (status.success || !status.f_parallel) {
            ...
            pos_sp->theta = ind > 0 ? compute_theta(*(path_r->dest[ind - 1]), *(path_r->dest[ind]))
                                    : compute_theta(*(path_r->dest[0]), *(path_r->dest[1]));
            prev_index = ind;
            is_init = false;
            prev_pos = *pos_sp;
            return 4;
