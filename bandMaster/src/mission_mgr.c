...
/**
 * @brief  return the path: set of consecutive vertices in Cartography containing those defined by
 * the mission
...
static void get_mission_path(Mission* mission, Cartography* carto, Path* path) {
    ...
    if (mission->size > 0) {
        ...
        path->dest[0] = &(carto->nodes[mission->ind[0]]); ///< Set the first element of path as the
                                                          ///< source of the mission

        /// find the shortest path of the defined mission by applying recursively Dijkstra between
        /// adjacent mission's vertices
 ...
 * @evolution receive some missions and order which one will be executed, consider the cancel of a
 * mission
 */
 ...
 * @evolution consider the canceling of a mission with Abort, also recompute a Path from the new
 * Mission
