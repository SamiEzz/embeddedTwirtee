 ...
 * trajectory contains the sequence of vertices between source and other sources: first element of
 * each row is the path size
 */
void dijkstra(Cartography* graph, int src, int dest, Path* trajectory) {
    Bool8 sptSet[MAX_CARTO_NODES]; // sptSet[i] will true if vertex i is included in path tree or
                                   // shortest distance from src to i is finalized
    int count, pt_index, node_index, i, u, v, v_index;
    ...
    for (count = 0; count < graph_size; ++count) {
        ...
        for (v = 0; v < graph->nodes[u].nb_a; ++v) {
            dist_uv = POINTDIST(*(graph->nodes + u), *(graph->nodes[u].arcs[v].dest)) /
                      get_speed(&(graph->nodes[u].arcs[v]), graph);
