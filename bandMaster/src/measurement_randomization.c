...
void random_gps(T_loc* gps, float freq_val, float noise_amplitude, float freq_noise) {
    ...
    if ((freq_noise > 1.0) || (freq_noise < 0.0) || (noise_amplitude < 0.0)) {
        sendMsg_CPU2("ERROR in RANDOM_GPS -> FREQ_NOISE must be in [0.0, 1.0] and NOISE_AMPLITUDE "
                     ">= 0.0!!!\n");
        return;
    }
...
void random_compass(T_head* compass, float freq_val, float noise_amplitude_deg, float freq_noise) {
    ...
    if ((freq_noise > 1.0) || (freq_noise < 0.0) || (noise_amplitude_deg < 0.0) ||
        (noise_amplitude_deg >= 360.0)) {
        sendMsg_CPU2("ERROR in RANDOM_COMPASS -> FREQ_NOISE must be in [0.0, 1.0] and "
                     "NOISE_AMPLITUDE must be in [0.0, 360.0]!!!\n");
        return;
    }
...
void random_odometry(T_odo* odometry, float freq_val, float noise_amplitude, float freq_noise) {
    ...
    if ((freq_noise > 1.0) || (freq_noise < 0.0) || (noise_amplitude < 0.0)) {
        sendMsg_CPU2("ERROR in RANDOM_ODOMETRY -> FREQ_NOISE must be in [0.0, 1.0] and "
                     "NOISE_AMPLITUDE >= 0.0!!!\n");
        return;
    }
    ...
    if ((random_value % FREQ_PRECISION) < freq_bound2) {
        odometry->l_speed =
            odometry->l_speed + random_int(-amplitude_bound, amplitude_bound) / 100.0;
        odometry->r_speed =
            odometry->r_speed + random_int(-amplitude_bound, amplitude_bound) / 100.0;
    }
}
...
void sensor_data_randomize(T_loc* gps, T_head* compass, T_odo* odometry, Bool8 randomize) {
    ...
    if (randomize) {
        ...
        random_compass(compass, input_random_param.invalid_compass_data,
                       input_random_param.compass_noise_amplitude,
                       input_random_param.noised_compass_data);
        /* GPS data randomization */
        random_gps(gps, input_random_param.invalid_gps_data, input_random_param.gps_noise_amplitude,
                   input_random_param.noised_gps_data);
        /* ODOMETRY data randomization */
        random_odometry(odometry, input_random_param.invalid_odometry_data,
